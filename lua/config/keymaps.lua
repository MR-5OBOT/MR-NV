...
vim.keymap.set("v", "<leader>ms", [[:s//g<Left><Left>]], { desc = "Multi-select & replace", noremap = true, silent = true })
...
local function run_python_in_tmux()
    ...
    local cmd = string.format("tmux split-window -h 'clear; python3 \"%s\"; echo; echo Press enter to exit...; read'", file)
    vim.fn.system(cmd)
end
